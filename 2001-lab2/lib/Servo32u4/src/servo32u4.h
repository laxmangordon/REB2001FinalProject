//servo32u4.h - Library for controlling servos on the ATmega32U4's hardware PWM pins (5, 6, 12, and 13)
#pragma once
 ...
 * 
 * Pins 6 and 12 use Timer4, but share the same output comp
 * are -- except that they are inverted.
...
protected:
    ...
    // NEW: ramp step size (microseconds per update call)
    uint16_t stepUs = 40;
...
public:
    ...
    // NEW: configure ramp smoothness
    void setStepUs(uint16_t step) { stepUs = (step == 0) ? 1 : step; }

    // NEW: immediate command (no ramp)
    void writeNow(uint16_t us)
    {
    ...
    void update(void)
    {
        ...
        else if (abs((int)targetPos - (int)currentPos) <= (int)stepUs) currentPos = targetPos;
        else if (targetPos > currentPos) currentPos += stepUs;
        else currentPos -= stepUs;
